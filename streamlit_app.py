...
[data-testid="stSidebar"] * { color: #ffffff !important; font-family: 'Barlow Condensed', sans-serif !important; }
[data-testid="stSidebar"] .stRadio label { font-size: 15px !important; font-weight: 600 !important; letter-spacing: 0.5px !important; color: #ffffff !important; padding: 6px 0 !important; }
[data-testid="stSidebar"] .stRadio > div { gap: 4px !important; }
[data-testid="stSidebar"] .stRadio [data-testid="stMarkdownContainer"] p { color: #ffffff !important; font-size: 15px !important; }
.sidebar-logo { background: linear-gradient(135deg, #e63946 0%, #c1121f 100%); margin: -1rem -1rem 1.5rem -1rem; padding: 1.5rem 1rem; text-align: center; }
.sidebar-logo h1 { font-family: 'Barlow Condensed', sans-serif !important; font-size: 22px !important; font-weight: 800 !important; color: #ffffff !important; letter-spacing: 2px !important; text-transform: uppercase !important; margin: 0 !important; line-height: 1.2 !important; }
.sidebar-logo p { color: rgba(255,255,255,0.8) !important; font-size: 11px !important; letter-spacing: 1px !important; margin: 4px 0 0 0 !important; text-transform: uppercase !important; }
.sidebar-team { background: rgba(230,57,70,0.1); border: 1px solid rgba(230,57,70,0.3); border-radius: 8px; padding: 10px 12px; margin-top: 12px; }
.sidebar-team p { color: #e63946 !important; font-size: 11px !important; font-weight: 600 !important; letter-spacing: 1px !important; text-transform: uppercase !important; margin: 0 0 4px 0 !important; }
.sidebar-team span { color: #ffffff !important; font-size: 13px !important; }
.main .block-container { padding: 1.5rem 2rem 2rem 2rem !important; max-width: 1400px !important; }
.hero-banner { background: linear-gradient(135deg, #e63946 0%, #c1121f 50%, #7b0d1e 100%); border-radius: 16px; padding: 2.5rem 2.5rem; margin-bottom: 2rem; position: relative; overflow: hidden; }
.hero-banner::before { content: '🏈'; position: absolute; right: 2rem; top: 50%; transform: translateY(-50%); font-size: 100px; opacity: 0.15; }
.hero-banner::after { content: ''; position: absolute; top: 0; right: 0; bottom: 0; width: 40%; background: repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(255,255,255,0.03) 10px, rgba(255,255,255,0.03) 20px); }
.hero-eyebrow { font-family: 'Barlow Condensed', sans-serif; font-size: 12px; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; color: rgba(255,255,255,0.7); margin-bottom: 8px; }
.hero-title { font-family: 'Barlow Condensed', sans-serif; font-size: 52px; font-weight: 800; color: #ffffff; line-height: 1.0; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 8px; }
.hero-subtitle { font-size: 16px; color: rgba(255,255,255,0.8); margin-bottom: 1.5rem; font-weight: 400; }
.stat-pills { display: flex; gap: 10px; flex-wrap: wrap; }
.stat-pill { background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.25); border-radius: 100px; padding: 6px 16px; font-family: 'Barlow Condensed', sans-serif; font-size: 14px; font-weight: 700; color: #ffffff; letter-spacing: 0.5px; }
.section-header { display: flex; align-items: center; gap: 12px; margin-bottom: 0.5rem; }
.section-number { background: #e63946; color: #ffffff; font-family: 'Barlow Condensed', sans-serif; font-size: 13px; font-weight: 800; letter-spacing: 1px; width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.section-title { font-family: 'Barlow Condensed', sans-serif; font-size: 32px; font-weight: 800; color: #ffffff; text-transform: uppercase; letter-spacing: 1px; line-height: 1; }
.section-subtitle { color: #9ca3af; font-size: 15px; margin-bottom: 1.5rem; font-style: italic; padding-left: 44px; }
.finding-card { background: linear-gradient(135deg, rgba(230,57,70,0.12) 0%, rgba(230,57,70,0.05) 100%); border: 1px solid rgba(230,57,70,0.4); border-left: 4px solid #e63946; border-radius: 10px; padding: 14px 18px; margin-top: 1rem; }
.finding-card p { color: #f1f5f9 !important; font-size: 14px !important; margin: 0 !important; line-height: 1.6 !important; }
.finding-label { font-family: 'Barlow Condensed', sans-serif; font-size: 11px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #e63946; margin-bottom: 4px; }
.metric-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin: 1.5rem 0; }
.metric-card { background: linear-gradient(135deg, #161b22 0%, #0d1117 100%); border: 1px solid #30363d; border-top: 3px solid #e63946; border-radius: 12px; padding: 20px; text-align: center; }
.metric-value { font-family: 'Barlow Condensed', sans-serif; font-size: 42px; font-weight: 800; color: #ffffff; line-height: 1; }
.metric-label { font-size: 12px; font-weight: 600; letter-spacing: 1.5px; text-transform: uppercase; color: #6b7280; margin-top: 4px; }
.sport-divider { height: 2px; background: linear-gradient(90deg, #e63946 0%, rgba(230,57,70,0.3) 50%, transparent 100%); margin: 2rem 0; border: none; }
h1, h2, h3, p, li, span { color: #ffffff; }
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] label div p { color: #ffffff !important; font-size: 15px !important; font-weight: 600 !important; }
...
if viz == "🏠  Overview":
    ...
    <div class="metric-row">
        <div class="metric-card"><div class="metric-value">16</div><div class="metric-label">Teams</div></div>
        <div class="metric-card"><div class="metric-value">400</div><div class="metric-label">Players</div></div>
        <div class="metric-card"><div class="metric-value">800</div><div class="metric-label">Matches</div></div>
        <div class="metric-card"><div class="metric-value">8</div><div class="metric-label">DB Tables</div></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    finding("No single factor decides who wins. Roster depth, injury management, and team execution matter more than payroll, age, or stadium size.")
    ...
    x = analytical["avg_win_pct"]; y = analytical["avg_games_missed"]
    m, b = np.polyfit(x, y, 1); x_line = np.linspace(x.min(), x.max(), 100)
    fig.add_trace(go.Scatter(x=x_line, y=m * x_line + b, mode="lines", name="Trend", line=dict(color="#f4a261", dash="dash", width=2)))
    ...
    cb1 = plt.colorbar(im1, ax=ax1); cb1.ax.yaxis.set_tick_params(color="#9ca3af"); cb1.set_label("Avg Fee ($M)", color="#9ca3af")
    ...
    cb2 = plt.colorbar(im2, ax=ax2); cb2.ax.yaxis.set_tick_params(color="#9ca3af"); cb2.set_label("Injuries per Transfer", color="#9ca3af")
    position_labels = {"CB": "Cornerback", "DL": "Defensive Lineman", "K": "Kicker", "LB": "Linebacker",
        "OL": "Offensive Lineman", "P": "Punter", "QB": "Quarterback", "RB": "Running Back",
        "S": "Safety", "TE": "Tight End", "WR": "Wide Receiver"}
    ...
    fig.legend(handles=legend_elements, loc="center left", bbox_to_anchor=(0.98, 0.95),
        title="Position Key", title_fontsize=9, prop={"size": 8}, frameon=True,
        handlelength=0, handletextpad=0, borderpad=0.5, ncol=1,
        facecolor="#161b22", labelcolor="#ffffff", title_fontcolor="#e63946")
    fig.suptitle("Transfer ROI by Position", fontsize=18, color="#ffffff", y=1.03, fontweight="bold")
    ...
    fig = px.scatter(df, x="avg_roster_age", y="points",
        ...
        hover_data={"season": True, "wins": True, "season_rank": True, "roster_size": True, "total_games_missed": True, "avg_roster_age": ":.1f"},
